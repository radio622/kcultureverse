...
## SI-7 — 멤버 노드 누락 (Member Node Missing)

**원인**: 그룹 노드는 존재하지만 핵심 멤버가 개인 노드로 등록되지 않아,
그룹과 멤버 간 MEMBER 엣지 자체가 없는 상태. 솔로 활동이 많거나
작곡/프로듀서로 타 아티스트와 협업이 많은 멤버일수록 파급력이 큼.

**확인된 사례:**
| 그룹 | 누락 멤버 | 영향 | 상태 |
|------|----------|------|------|
| 산울림 | 김창완 | 산울림 ↔ 김창완 연결 없음, 김창완 솔로 작업과의 교차 단절 | 🔲 미수정 |
| (전수조사 필요) | | | 🔲 미완 |

**전수조사 방법:**
```python
# 그룹 노드는 있는데 MEMBER 엣지 수가 0인 경우 탐지
for n in nodes:
    if n.get('type') == 'group':
        member_edges = [e for e in edges if
            (e['source']==n['mbid'] or e['target']==n['mbid'])
            and e['relation'] in ('MEMBER', 'SAME_GROUP')]
        if len(member_edges) == 0:
            print(f"멤버 엣지 없음: {n['name']}")
```

**우선 확인 필요 그룹 (역사적으로 중요하나 멤버 누락 가능성 높음):**
- 산울림 (김창완, 김창훈, 김창익)
