...
| 역할/등급 | 조건 | 권한 범위 (유니버스 경험) | 데이터 에디트 제안 (AI 검증) |
|--------|------|-----------------------|-------------------------|
| **Admin** | 관리자 계정 | 무제한, 전체 관리자 패널 접근 | ⭕ 가능 (제한 없음, 즉시 반영) |
| **정회원** (Full) | 가입 + 뉴스레터 동의 | 🚀 우주 자율주행 피쳐 사용 가능 | ⭕ 가능 (1분에 1회 제한) |
| **준회원** (Associate) | 가입 + 뉴스레터 미동의 | 🚀 우주 자율주행 피쳐 사용 가능 | ❌ 불가능 (열람/탐색만 가능) |
| **비회원** (Guest) | 미로그인 | 기본 우주 탐색 및 음악 재생만 가능 | ❌ 불가능 |

> **설계 의도**: 유저 참여형 시스템 구축 시 무분별한 데이터를 막기 위해, "내가 좋아하는 아티스트를 우주에 올리려면, 최소한 뉴스레터를 수신하고 우주 소식을 듣겠다는 의지(정회원)"를 보여주어야만 에디트 권한을 부여합니다.
