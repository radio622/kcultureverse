...
|------|------|-------------------|------|
| **GPT-5 Nano** | 메인 게이트키퍼 (유료) | Input: **$0.05/1M tokens**<br>Output: **$0.40/1M tokens** | 400K 컨텍스트, Function Calling 지원, 분류/요약에 최적화 |
| **Gemini 3.1 Flash-Lite** | 메인 게이트키퍼 (무료 운영 시) | 유료: Input $0.25 / Output $1.50 per 1M tokens<br>**무료 티어: Google AI Studio에서 제한적 무료 사용 가능** (Preview, 분당 ~5~15 req, 일 ~5건 내외) | Preview 상태이나 Google AI Studio 무료 계정으로도 소규모 호출 가능. 초기 운영 시 완전 무료 운영 가능한 핵심 옵션 |
| **Gemini 2.5 Flash-Lite** | Fallback (안정적 무료 백업) | 무료 (Free Tier, 분당 15 req, 일 1,500건+) | 안정 버전(Stable). 3.1보다 Rate Limit이 넉넉하여 3.1 쿼터 소진 시 자동 전환용 |

> 💡 **무료 운영 전략**: 초기에는 Gemini 3.1 Flash-Lite 무료 티어만으로 운영하다가, 유저 수가 늘어 일일 요청이 무료 한도(~5건)를 초과할 경우 GPT-5 Nano(월 $1 미만)로 전환하거나 유료 Tier 1($0.25/1M tokens)으로 업그레이드합니다. Gemini 2.5 Flash-Lite는 어떤 경우에도 항상 안정적인 무료 백업으로 대기합니다.
...
## 8. Phase별 개발 계획 (Implementation Phases)
...
## 9. 최종 아키텍처 다이어그램 (Architecture Overview)
