...
| 외부 AI API | 없음 | GPT-5 Mini (Admin) / Gemini 3.1 Flash-Lite (일반 유저) |
...
- **로그인 후 UI 보존**: 광활한 우주 탐험을 방해하지 않기 위해, 로그인 상태라도 거추장스러운 대시보드가 뜨지 않습니다. **우측 상단 모서리에 작고 깔끔한 유저 프로필 아이콘(또는 아바타) 하나만 띄워** 몰입감을 유지합니다. 아이콘 클릭 시 드롭다운 메뉴(마이페이지, 에디트 제안, 자율주행, 로그아웃)가 나옵니다.
- **홈페이지 자연스러운 안내**: 첫 진입 시, 하단에 작게 "가입하면 나만의 우주 비행(자율주행)과 아티스트 추가 요청이 가능해요!" 같은 토스트 알럿을 띄워 자연스럽게 가입을 유도합니다.

### 2.2 권한 및 멤버십 분류

| 역할/등급 | 조건 | 권한 범위 (유니버스 경험) | 데이터 에디트 제안 (AI 검증) | 게시판 열람 권한 |
|--------|------|-----------------------|-------------------------|----------------|
| **Admin** | 관리자 계정 | 무제한, 전체 관리자 패널 접근 | ⭕ 가능 (제한 없음, 즉시 반영) | ⭕ 모든 유저의 요청 조회/삭제/승인 |
| **정회원** (Full) | 가입 + 뉴스레터 동의 | 🚀 우주 자율주행 피쳐 사용 가능 | ⭕ 가능 (1분에 1회 제한) | ⭕ 모든 요청 조회 가능, 내 요청 수정 |
| **준회원** (Associate) | 가입 + 뉴스레터 미동의 | 🚀 우주 자율주행 피쳐 사용 가능 | ❌ 불가능 (열람/탐색만 가능) | ⭕ 모든 요청 조회 가능 |
| **비회원** (Guest) | 미로그인 | 기본 우주 탐색 및 음악 재생만 가능 | ❌ 불가능 | ❌ 불가능 (접근 불가) |

> **설계 의도**: 유저 참여형 시스템 구축 시 무분별한 데이터를 막기 위해, "내가 좋아하는 아티스트를 우주에 올리려면, 최소한 뉴스레터를 수신하고 우주 소식을 듣겠다는 의지(정회원)"를 보여주어야만 에디트 권한을 부여합니다. 뉴스레터를 나중에 해제하면 정회원에서 준회원으로 강등되며 에디트 권한이 즉시 정지됩니다(에디트 제출 시점에 `newsletter=true` 매번 검증). 유저 요청 게시판은 로그인된 회원(준회원 이상)에게만 개방되어 투명하게 정보를 공유하지만, 비회원에게는 숨겨서 불필요한 스팸 노출을 피합니다.
...
**일반 유저 (Gemini 3.1 Flash-Lite)**: 무료 티어로 운영. 비용 $0.

**Admin (GPT-5 Mini)**: Admin만 사용하므로 호출 빈도 극히 낮음.

| 시나리오 | Admin 일일 요청 수 | 요청 당 토큰 (In/Out) | 월간 비용 |
|---------|-------------------|---------------------|----------|
| 초기 (소규모) | 5건 | ~800 / ~400 | **$0.07** (거의 무료) |
| 활발한 관리 | 20건 | ~1,000 / ~600 | **$0.55** |
| 대규모 교정 | 50건 | ~1,000 / ~600 | **$1.38** |

> 💡 Admin 전용 GPT-5 Mini($0.80/1M input, $3.20/1M output)는 Nano보다 비싸지만, Admin 호출 빈도가 매우 낮으므로 **월 $2 미만**으로 운영 가능합니다. 일반 유저는 Gemini 무료 티어로 비용 $0입니다.
...
### 3.4 검증 전략: LLM + 서버 사이드 병행

> ⚠️ **설계 변경**: Gemini 3.1 Flash-Lite는 경량 모델이라 Function Calling(Tool Use) 지원이 제한적일 수 있습니다. 따라서 일반 유저 요청의 검증은 **LLM에게 Tool을 위임하지 않고, 서버 사이드 코드에서 직접 API를 호출**하는 하이브리드 방식으로 변경합니다.

```
[일반 유저 흐름 — Gemini 3.1 Flash-Lite]
  1. LLM이 자연어 입력을 파싱 → Intent + 구조화 데이터(아티스트명, 관계 등) 반환
  2. 서버 코드에서 직접 검증 API 호출 (Promise.all 병렬):
     ├─ MusicBrainz API: MBID 존재 확인, 크레딧 교차 검증
     ├─ iTunes API: 아티스트 존재 확인, 트랙 데이터 확인
     └─ (선택) Google CSE API: 웹 검색 교차 검증
  3. 검증 결과를 종합하여 서버 코드가 최종 판정 (Approved/Pending/Rejected)

[Admin 흐름 — GPT-5 Mini]
  1. GPT-5 Mini는 Function Calling 완전 지원, but 검증 자체를 스킵
  2. LLM이 자연어 → 구조화 JSON 파싱만 수행
  3. 서버 코드가 organic-graph.json에서 아티스트명 → MBID 매핑 후 즉시 data_overrides 등록
```

이 방식의 장점:
- Flash-Lite의 Tool 지원 여부에 **의존하지 않음**
- 검증 로직이 서버 코드에 있으므로 **테스트 가능하고 안정적**
- API 호출 로직을 한곳에서 관리하므로 **유지보수 용이**
```
...
CREATE TABLE user_profiles (
  ...
  email         TEXT NOT NULL,
  nickname      TEXT UNIQUE NOT NULL,                     -- 게시판 노출용 닉네임 (중복 불가)
  gender        TEXT CHECK (gender IN ('male','female','other','undisclosed')),
  age_group     TEXT CHECK (age_group IN ('10s','20s','30s','40s','50s+')),
  newsletter    BOOLEAN DEFAULT false,
  role          TEXT DEFAULT 'user' CHECK (role IN ('admin','user')),
  membership    TEXT DEFAULT 'associate' CHECK (membership IN ('associate','full')), -- 준/정회원 구분
...
CREATE TABLE universe_edit_logs (
  ...
  user_id       UUID REFERENCES user_profiles(id) ON DELETE SET NULL,  -- 탈퇴 시 익명화
...
-- 자율주행 비행 기록 (V7.0 Premium)
CREATE TABLE flight_logs (
  id            SERIAL PRIMARY KEY,
  user_id       UUID REFERENCES user_profiles(id) ON DELETE CASCADE,   -- 탈퇴 시 비행기록 완전 삭제
  start_artist  TEXT NOT NULL,           -- 출발 아티스트 이름
  path          JSONB NOT NULL,          -- [{id, name, trackName?, duration}]
  total_stops   INT NOT NULL DEFAULT 0,  -- 총 방문 아티스트 수
  total_seconds INT NOT NULL DEFAULT 0,  -- 총 비행 시간 (초)
  created_at    TIMESTAMPTZ DEFAULT NOW()
);
```
...
| 항목 | V6.9 현재 | V7.0 예상 (1년 후) | V7.x 장기 (2년 후) |
|------|----------|-------------------|-------------------|
...
| v5-details.json | **388KB** | ~800KB | ~2MB |
...
## 6. 지원되는 관계 모델 확장 (Expanded Relationship Models)

유저(정회원) 제안 및 Admin 빠른 수정을 통해 유니버스의 서사를 훨씬 풍부하게 만들어 줄 **신규 간접 관계(Indirect)** 가중치 스펙입니다. 너무 강한 가중치는 우주를 뭉치게(Hairball) 하므로, 약한 가중치(0.1 ~ 0.3)로 설정하여 독립성을 유지하면서 부드럽게 당깁니다.

### 6.1 크레딧 외 간접 음악적 관계 (INDIRECT)
노드(아티스트)로 등록되지 않은 동일 인물(작사가, 작곡가 등)의 곡을 공유하는 경우:
- **1곡 겹침**: Weight 0.1
- **2곡 겹침**: Weight 0.2
- **3곡 이상 겹침**: Weight 0.3 (Max)
> *사례: 김이나 작사가의 곡을 부른 아이유와 가인 → INDIRECT (0.2)*

### 6.2 같은 기획사/레이블 소속 (LABEL)
- **동기간 소속 (같은 시기 활동)**: Weight 0.2
- **불분명 / 단기 계약**: Weight 0.15
- **다른 기간 소속**: Weight 0.1
> *사례: 2010년대 SM 소속이었던 동방신기, 슈퍼주니어, 소녀시대 등 → LABEL (0.2)*

### 6.3 같은 방송/미디어 공동 출연 (TV_SHOW)
- **2회 이상 공동 출연 (고정 멤버 등)**: Weight 0.3
- **1회성 공동 출연 (게스트 등)**: Weight 0.2
- **불분명**: Weight 0.15
- **같은 프로 다른 시간대 출연**: Weight 0.1
> *사례: 무한도전 가요제에 같이 출연했던 혁오와 자이언티 → TV_SHOW (0.2)*

---

## 7. 기술적 과제 상세 분석 (Technical Challenges)

### 7.1 LLM 환각 (Hallucination) 방어
...
| 탭 | 기능 | 설명 |
|----|------|------|
| **🚀 빠른 수정** | AI 직통 입력창 | 자연어로 아티스트 정보/관계를 입력하면 **사실검증 생략**, GPT-5 Mini가 즉시 데이터 포맷으로 파싱하여 `data_overrides`에 직접 등록. Admin 전용 고속 수정 채널 |
...
| **API 사용량 모니터** | GPT-5 Mini / Gemini의 이번 달 총 토큰 사용량 및 예상 비용 실시간 표시 | 🟢 낮음 |
...
### 9.1 끊김 없는 무한 릴레이 미리듣기 (Continuous Preview)
...
### 9.2 우주 자율주행 (Autonomous Universe Exploration)
...
## 10. Phase별 개발 계획 (Implementation Phases)

### Phase 1: 인증 및 DB 기반 구축 (예상 1~2주)
- [ ] Supabase 프로젝트 생성 및 4개 테이블 (`user_profiles`, `universe_edit_logs`, `data_overrides`, `flight_logs`) 마이그레이션
- [ ] Auth.js v5 연동 및 구글 소셜 로그인 구축
- [ ] 온보딩 모달 구현: 닉네임, 성별, 연령대, 뉴스레터 선택 폼. 뉴스레터 미동의 시 "정회원은 아티스트 정보 업데이트가 가능해요!" 안내 표출.
- [ ] 미니멀 로그인 UI: 우측 상단 유저 아바타 표시, 클릭 시 드롭다운 (에디트 제안, 마이페이지, 로그아웃)
- [ ] 마이페이지 모델: 닉네임 변경, 뉴스레터 수신 상태 변경(정/준회원 승급·강등), **회원탈퇴 기능**(소셜 연결 해제 및 개인정보 즉각 폐기 시스템)
- [ ] 첫 방문 유저 안내 토스트 메시지 개발 (유니버스 탐색 중 슬쩍 떴다 사라짐)
- [ ] Admin 계정 게이트 구현 (환경변수 `ADMIN_EMAILS` 및 `ADMIN_PASSPHRASE`)
- [ ] 개인정보 처리방침 페이지 작성 + 온보딩 시 동의 체크박스 (한국 PIPA 대응)

### Phase 2: Admin 관리자 대시보드 (예상 1~2주)
- [ ] `/admin` 라우트 레이아웃 및 탭 네비게이션 (빠른 수정 / 유저 요청 / 회원 관리 / 통계 / 빌드 제어 / 롤백)
- [ ] 🚀 빠른 수정 탭: GPT-5 Mini AI 직통 입력창 (사실검증 스킵, 즉시 `data_overrides` 등록)
...
- [ ] 유저 미인증 시 로그인 유도 플로우. 준회원이 에디트 누를 시 "정회원만 가능해요" 안내 팝업.

### Phase 4: AI 에이전트 엔진 (예상 2~3주)
- [ ] Vercel AI SDK v5 설치 및 Gemini 3.1 Flash-Lite(일반 유저) + GPT-5 Mini(Admin) Provider 연동
- [ ] 시스템 프롬프트(System Prompt) 정밀 작성 — Intent 분류, 검증 기준, 출력 JSON 스키마 정의
- [ ] **Admin 전용 프롬프트** 별도 작성 — 검증 스킵, 데이터 파싱만 수행
- [ ] 서버 사이드 검증 API 3종 구현 (`search_musicbrainz`, `search_itunes`, `search_web`) — LLM에게 Tool을 위임하지 않고 서버 코드에서 직접 호출
- [ ] 판정 결과(Approved/Pending/Rejected)를 DB에 저장하고 유저에게 실시간 피드백하는 API 파이프라인 완성
- [ ] Gemini 2.5 Flash-Lite 무료 백업 Provider 추가 (3.1 장애/쿼터 소진 시 자동 전환)
...
- [ ] `useAudioQueue` 커스텀 훅 구현 — iTunes Search API로 아티스트 Top Tracks 큐 관리
- [ ] 프리로드 방식 Gapless Playback: 현재 곡 재생 중 다음 곡 `new Audio(nextUrl)` 미리 로드
- [ ] 자율주행(Auto-Warp) 엔진: 가중치 기반 확률적 경로 탐색 + Tabu List(최근 5개 노드)
- [ ] 자율주행 시작 UI: "자율주행 시작 🚀" 버튼 + 시작 아티스트 선택 플로우
- [ ] Flight Log 브레드크럼 UI: 실시간 방문 경로 표시 + 종료 시 Supabase `flight_logs` 저장
- [ ] previewUrl 없는 노드 스킵 로직: 시각적 워프만 수행 → 3초 후 자동 다음 이웃
- [ ] 클러스터 고착 방지: Tabu List 동적 확장(5→10) + 강제 랜덤 점프 5% 확률
- [ ] 로그인 유저 전용 잠금(Lock-up) 게이트: 미인증 시 "로그인하면 자율주행 가능!" 유도

---

## 11. 최종 아키텍처 다이어그램 (Architecture Overview)
...
│     │    ├─ Vercel AI SDK → Gemini 3.1 Flash-Lite        │
│     │    │    ├─ LLM: Intent 분류 + 파싱만                │
│     │    │    ├─ 서버: search_musicbrainz() 직접 호출      │
│     │    │    ├─ 서버: search_itunes() 직접 호출           │
│     │    │    └─ 서버: search_web() 직접 호출 (선택)       │
│     │    └─ 판정 결과 → Supabase DB                       │
│     ├─ POST /api/admin/quick-edit (Admin 직통)            │
│     │    ├─ Admin 암호 검증                                │
│     │    ├─ Vercel AI SDK → GPT-5 Mini (검증 없이 파싱만)  │
...
## 12. 위험 요소 요약 (Risk Assessment)

| 위험 | 확률 | 영향 | 완화 수단 |
|------|------|------|----------|
| LLM 환각으로 잘못된 데이터 승인 | 중간 | 높음 | 시스템 프롬프트 하드코딩 + 출처 2건 이상 필수 |
| GPT-5 Mini API 장애/쿼터 소진 | 낮음 | 중간 | Gemini 2.5 Flash-Lite 자동 Fallback |
...
| 자율주행 중 previewUrl 없는 노드 도달 | 중간 | 낮음 | 시각적 워프만 수행 + 3초 후 자동 스킵 |
| Apple iTunes API 과다 호출 차단 | 낮음 | 중간 | SessionStorage 캐시 + 분당 10회 이하 제한 |
| 자율주행이 좁은 클러스터에서 벗어나지 못함 | 중간 | 낮음 | Tabu List 동적 확장 + 강제 랜덤 점프 5% 확률 |

---

> **다음 단계**: V6.9 안정화를 마무리한 뒤, Phase 1(인증 및 DB 구축)부터 착수합니다.
