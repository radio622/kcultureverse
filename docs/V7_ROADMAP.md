...
> **설계 의도**: 유저 참여형 시스템 구축 시 무분별한 데이터를 막기 위해, "내가 좋아하는 아티스트를 우주에 올리려면, 최소한 뉴스레터를 수신하고 우주 소식을 듣겠다는 의지(정회원)"를 보여주어야만 에디트 권한을 부여합니다. 유저 요청 게시판은 로그인된 회원(준회원 이상)에게만 개방되어 투명하게 정보를 공유하지만, 비회원에게는 숨겨서 불필요한 스팸 노출을 피합니다.
...
CREATE TABLE user_profiles (
  ...
  email         TEXT NOT NULL,
  nickname      TEXT NOT NULL,                           -- 게시판 노출용 닉네임 추가
